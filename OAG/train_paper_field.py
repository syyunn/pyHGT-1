...
parser = argparse.ArgumentParser(
    description="Training GNN on Paper-Field (L2) classification task"
)
...
parser.add_argument("--n_epoch", type=int, default=200, help="Number of epoch to run")
parser.add_argument(
...
graph = renamed_load(
    open(os.path.join(args.data_dir, "graph%s.pk" % args.domain), "rb")
)
...
def prepare_data(pool):
    ...
    for batch_id in np.arange(args.n_batch):
        p = pool.apply_async(
            node_classification_sample, args=(randint(), sel_train_pairs, train_range)
        )
        jobs.append(p)
    p = pool.apply_async(
        node_classification_sample, args=(randint(), sel_valid_pairs, valid_range)
    )
    jobs.append(p)
    return jobs
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    with torch.no_grad():
        ...
        print(
            (
                "Epoch: %d (%.1fs)  LR: %.5f Train Loss: %.2f  Valid Loss: %.2f  Valid NDCG: %.4f"
            )
            % (
                epoch,
                (st - et),
