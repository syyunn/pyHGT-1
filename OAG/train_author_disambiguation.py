...
parser = argparse.ArgumentParser(
    description="Training GNN on Author Disambiguation task"
)
...
parser.add_argument("--n_epoch", type=int, default=100, help="Number of epoch to run")
parser.add_argument(
    "--n_pool", type=int, default=4, help="Number of process to sample subgraph"
)
parser.add_argument(
    "--n_batch",
    type=int,
...
parser.add_argument(
    "--batch_size", type=int, default=256, help="Number of output nodes for training"
)
...
for i, j in tqdm(
    graph.node_feature["author"].iterrows(), total=len(graph.node_feature["author"])
):
    if i in first_author_dict:
        name_count[j["name"]] += [i]
name_count = {
    name: name_count[name] for name in name_count if len(name_count[name]) >= 4
}
...
def author_disambiguation_sample(seed, pairs, time_range, batch_size):
    ...
    for name in names:
        ...
        for p_id, author_id, _time in pairs[name]:
            ...
            """
            name_label += [
                [author_dict[author_list[author_id]]]
                + [
                    author_dict[a_id]
                    for (x_id, a_id) in enumerate(author_list)
                    if x_id != author_id
                ]
            ]
    ...
    for x_id, author_ids in enumerate(name_label):
        ylabel[x_id + node_dict["paper"][0]] = (
            np.array(author_ids) + node_dict["author"][0]
        )
...
def prepare_data(pool):
    ...
    for batch_id in np.arange(args.n_batch):
        p = pool.apply_async(
            author_disambiguation_sample,
            args=(randint(), sel_train_pairs, train_range, args.batch_size),
        )
        jobs.append(p)
    p = pool.apply_async(
        author_disambiguation_sample,
        args=(randint(), sel_valid_pairs, valid_range, args.batch_size),
    )
    jobs.append(p)
    return jobs
...
for name in name_count:
    ...
    for author_id, author in enumerate(same_name_author_list):
        for p_id in graph.edge_list["author"]["paper"]["rev_AP_write_first"][author]:
            _time = graph.edge_list["author"]["paper"]["rev_AP_write_first"][author][
                p_id
            ]
            if type(_time) != int:
                continue
...
sel_train_pairs = {
    ...
    for p in np.random.choice(
        list(train_pairs.keys()),
        int(len(train_pairs) * args.data_percentage),
        replace=False,
    )
}
sel_valid_pairs = {
    p: valid_pairs[p]
    for p in np.random.choice(
        list(valid_pairs.keys()),
        int(len(valid_pairs) * args.data_percentage),
        replace=False,
    )
}
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    with torch.no_grad():
        ...
        if valid_ndcg > best_val:
            ...
            torch.save(
                model,
                os.path.join(args.model_dir, args.task_name + "_" + args.conv_name),
            )
            print("UPDATE!!!")
        ...
        print(
            (
                "Epoch: %d (%.1fs)  LR: %.5f Train Loss: %.2f  Valid Loss: %.2f  Valid NDCG: %.4f  Valid MRR: %.4f"
            )
            % (
                epoch,
                (st - et),
...
with torch.no_grad():
    ...
    for _ in range(10):
        (
            ...
            randint(), test_pairs, test_range, args.batch_size
        )
        node_rep = gnn.forward(
            node_feature.to(device),
            node_type.to(device),
